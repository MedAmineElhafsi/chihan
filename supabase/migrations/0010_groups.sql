-- 0010_groups.sql — local community groups for the diaspora
...
-- Stats view (member count). security_invoker so RLS still applies on base tables.
...
-- Owner bootstrap: creator inserts themselves as owner (allowed by insert policy).
